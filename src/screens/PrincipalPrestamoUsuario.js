...
const SolicitarPrestamoUsuario = () => {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .select(`
          ...
          horaprestamo,
          estado,
          equipos:equipos(nombreequipo, modelo, numero_serie),
      ...
      const prestamosFormateados = data?.map(item => ({
        ...item,
        fechaCompleta: item.fechaprestamo 
  ...
  if (Platform.OS === 'android') {
    return (
      <DrawerLayoutAndroid
        ...
        drawerWidth={300}
        drawerPosition="left"
        renderNavigationView={() => <MenuUsuario navigation={navigation} />}
      >
